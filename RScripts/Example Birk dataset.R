...
###############################################################################
###############################################################################  
  glimpse(met[met$case == 167, ])
  
  sub.dat <- dat[dat$case == 167, ]
  
  # apply the Box-Cox transformation  
  P.y = estimateBC(sub.dat$y)
  y = applyBC(sub.dat$y, P.y)
  
  P.x1 = estimateBC(sub.dat$x1)
  x1 = applyBC(sub.dat$x1, P.x1)
  
  P.x2 = estimateBC(sub.dat$x2)
  x2 = applyBC(sub.dat$x2, P.x2)
  ...
  vis.gam(m2, plot.type = "persp", color = "terrain", 
          theta = 140, phi = 25, too.far = tf, zlim = c(z1, z2), n.grid = 20,
          main = expression(paste(bold(b), "    y = s(", x[1], ") + s(", x[2],
                                  ") + ", beta[3], x[1], x[2])), zlab = "y")
  
  vis.gam(m3, plot.type = "persp", color = "terrain", 
          theta = 140, phi = 25, too.far = tf, zlim = c(z1, z2), n.grid = 20,
          main = expression(paste(bold(c), "    y = s(", x[1], ") + s(", x[2],
                                  ") + ti(", x[1], ",", x[2], ")")), zlab = "y")
