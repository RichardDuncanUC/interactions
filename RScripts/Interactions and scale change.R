...
# plot where there is no interaction on the log scale
  ...
  plot(ly2 ~ x1, type = "l", bty = "l", lwd = 2, ylim = c(-1, 2.2),
       xlab = expression(x[1]), ylab = "log(y)", cex.lab = 1.5)
  lines(ly1 ~ x1, lwd = 2)
  
  mtext("a", line = 1, adj = 0, cex = 1.3)
  mtext(expression(paste("log(y) = ",beta[0]," + ",beta[1],x[1]," + ",beta[2],x[2])), line = 2, cex = 1.3)
  
  text(10, max(ly1 + 0.2), expression(paste(x[2], " = 1")), xpd = NA, cex = 1.3)
  text(10, max(ly2 + 0.2), expression(paste(x[2], " = 10")), xpd = NA, cex = 1.3)  
  
  
  plot(y2 ~ x1, type = "l", bty = "l", lwd = 2, ylim = c(0, 8),
       xlab = expression(x[1]), ylab = "y", cex.lab = 1.5)
    lines(y1 ~ x1, lwd = 2)
...
# plot where there is no interaction on the untransformed scale  
  ...
  mtext(expression(paste("y = ",beta[0]," + ",beta[1],x[1]," + ",beta[2],x[2])), line = 2, cex = 1.3)
  
  text(100, max(y1 + 1), expression(paste(x[2], " = 1")), xpd = NA, cex = 1.3)
  text(100, max(y2 + 1), expression(paste(x[2], " = 100")), xpd = NA, cex = 1.3)
